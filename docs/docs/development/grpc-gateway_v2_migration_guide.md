...
This guide is supposed to help users of the gateway migrate from v1 to v2. See [the original issue](https://github.com/kwens/grpc-gateway/issues/1223) for detailed information on all changes that were made specifically to v2.

The following behavioural defaults have been changed:

## protoc-gen-swagger has been renamed protoc-gen-openapiv2

See [the original issue](https://github.com/kwens/grpc-gateway/issues/675)
...
See [a_bit_of_everything.proto](https://github.com/kwens/grpc-gateway/blob/main/examples/internal/proto/examplepb/a_bit_of_everything.proto) in the example protos for more examples.

## We now use the camelCase JSON names by default

See [the original issue](https://github.com/kwens/grpc-gateway/issues/375) and
[original pull request](https://github.com/kwens/grpc-gateway/pull/540) for more information.
...
See [the original issue](https://github.com/kwens/grpc-gateway/issues/233)
...
the default behaviour. See [the original issue](https://github.com/kwens/grpc-gateway/issues/224) for more information.
...
The default behaviour for query parameter parsing has changed to return an `InvalidArgument` (`400 Bad Request`) error when more than one of the same matching query parameters is parsed. Previously, it would log but not return an error, using the first query parameter that matched and ignoring any others. See [the original issue](https://github.com/kwens/grpc-gateway/issues/2632) for more information.
